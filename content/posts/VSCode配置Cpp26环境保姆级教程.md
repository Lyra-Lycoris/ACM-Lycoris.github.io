...
draft: false
categories: ["教程"]
description: "面向零基础新生：装 VS Code、装 GCC、配环境变量、装插件、按 F6 一键编译运行并弹出独立窗口，最后装 CPH 打比赛。开头附一键懒人包，国内直链无需代理。"
tags: ["VS Code", "C++", "GCC", "C++26", "CPH", "环境配置", "教程", "竞赛"]
---

> 写给完全没碰过编程的新生。
>
> **不想折腾的，看完第零节就可以关了**，双击一个文件全自动装完。
>
> 想自己一步步装的，从第一节开始往下抄，**每一步都告诉你点哪里、粘什么命令**。全文没有需要你自己动脑的地方，照做就行。
